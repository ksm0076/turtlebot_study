...
class DistanceTracker(Node):
    def __init__(self):
        ...
        ##
        self.time_pub = self.create_publisher(String, '/time_print', 10)
        self.distance_pub = self.create_publisher(String, '/distance_print', 10)
    ...
    def show_distance(self):
        if self.is_measuring_distance:
            ...
            pub_time = str(self.minute).zfill(2) + ":" + str(self.second).zfill(2)
            pub_distance = str(f"{self.total_distance:.2f}")
            self.time_pub.publish(String(data=pub_time))
            self.distance_pub.publish(String(data=pub_distance))
